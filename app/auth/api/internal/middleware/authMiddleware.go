...
func (m *AuthMiddleware) Handle(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 校验Permission
		ret1, err := m.UserRoleRPC.GetUserRoleByUserId(ctx, &pb.GetUserRoleByUserIdReq{UserId: userIdInt64})
		if err != nil {
			httpx.ErrorCtx(ctx, w, xcode.Unauthorized)
			return
		}
		...
		for _, userRole := range ret1.UserRole {
			...
			ret2, err := m.RolePermissionRPC.GetRolePermissionByRoleId(ctx, &pb.GetRolePermissionByRoleIdReq{RoleId: roleId})
			if err != nil {
				httpx.ErrorCtx(ctx, w, xcode.Unauthorized)
				return
			}
